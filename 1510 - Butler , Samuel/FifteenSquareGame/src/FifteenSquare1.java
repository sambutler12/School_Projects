...
public class FifteenSquare1
{
	...
	//also made a variable to keep the last move that they entered in order to do the undo command
	...
	public static void main(String[] args)
	{
		...
		do
		{
			/*This is the loop that makes the 3 by 3 array of integers by sending it to the make board method
			...
			do
			{
				...
				//makes sure that if the user just hits enter and no specific direction that
	...
	public static char checkMove(int[][] board,char move)
	{
		...
		{ 
			...
			{
				System.out.println("You can not use undo right now. Try again. ");
				movingTile = true;
			}
		}
	...
	public static int column(int c)
	{
		 ...
		 *to one of them and return that column that the number is in which is either a 0, 1, 2, or 3
		 *					  0            1           2           3
		 ...
		 *					| 12 |       | 13 | 	| 14 | 	    | 15 | 
		 *
		 *The blank = ? (equal to some number), which is number in a certain column
		 *
		 *Row 0 has numbers 0, 4,  8, 12
		 *Row 1 has numbers 1, 5,  9, 13
		 *Row 2 has numbers 2, 6, 10, 14
		 *Row 3 has numbers 3, 7, 11, 15
		 */
	...
	public static boolean moveBlank(int[][] board, char blank, int row, int column)
	{
		 ...
		 *is equal to one of these cases. If it is, it will swap it with the number that is to its 
		 *right or left, or above or below it. It will also checks if it is able to move in the direction 
		 *that the user has chosen by seeing if it is up against a wall. If they try to move towards the wall
		 * the moveBlank will return false and prompt them again.
		 * 
		 * To see if it is against a wall we check it based on the move that they entered and where the blank
		 * is currently on the board. 
		 * 
		 * EXAMPLE:
		 * Say the blank is at position [0][3] and the user inputs 'U' meaning they want to go up, well we all know
		 * that they can't go up or they would go off the board, so by going to the if statement that compares 'U', 
		 * it will look at the row and see's if it is able to go up, if the row is already equal to zero 
		 * it says "hey you are already at row zero there's no where to go up too" and will return false and ask them for
		 * a new move. All the other cases work the same way by looking at the row for 'U' and 'D' and looking at the column
		 * for 'L' and 'R' to make sure that they can't move off the board
		 */
	...
	public static int[][] shuffle(int[][] board)
	{
		...
		while(movesMade <= 10000)
		{
			...
			//This checks to see if it is the inverse of a move, if it is then it will discard it
			if(movesMade > 0)
			{

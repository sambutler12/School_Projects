...
public class ProcessTextFile
{
	public static void main(String[] args)
	{
		...
		// Array of the text files so they can be all ran at once, easy when it comes to collecting the data and not having to type the 
		// files name every time. 
		final String[] VariousFiles = {"Anna Karenina - Tolstoy"              , "Bleak House - Dickens"              , "David Copperfield - Dickens"  ,
									   "Don Quixote - Cervantes"              , "Green Eggs and Ham"                 , "Hamlet"                       , 
									   "Hamlet-Scene-1"                       , "King James Bible"                   , "Les Miserables - Hugo"        ,
									   "Middlemarch - Eliot"                  , "Moby Dick - Melville"               , "Shakespeare"                  ,
									   "The Brothers Karamazof - Dostoyevsky" , "The Hunchback of Notre Dame - Hugo" , "The Three Musketeers - Dumas" ,
									   "To Kill a Mockingbird - Harper Lee"   , "War and Peace"                      , "ALL"};
		
		int i = 0;	// location of text file
		
		while(i < 18)	// when i is 18 then that means it ran through all the files and will terminate
		{
			//final String FILE_NAME = "Hamlet";		// incase you want to go one by one to  each file
			final String FILE_NAME = VariousFiles[i];	// increments i after it is finished with a text file
			
			File file	= new java.io.File("F:\\2500 - Butler, Samuel\\2500-Lab4\\TestTextFiles-Lab4\\" + FILE_NAME + ".txt");
			...
			try
			{
				...
				System.out.println(VariousFiles[i]);	// Prints out the file name, so i know what stats go to which file
				System.out.println();
				SeventhPassHash(file);
			}
			...
			i++;	// increments to the next text file
		}
	...
	public static void SecondPassUnsorted(File file) throws FileNotFoundException
	{
		...
		Scanner input = new Scanner(file);
		ListUnsorted unsorted	= new ListUnsorted();
	...
	public static void ThirdPassSorted (File file) throws FileNotFoundException
	{
		...
		Scanner input = new Scanner(file);
		ListSorted sorted = new ListSorted();			// instance of the old sorted list
	...
	public static void FourthPassToFront(File file) throws FileNotFoundException
	{
		...
		Scanner input = new Scanner(file);
		HeavyHandedList adjFront	= new HeavyHandedList();
	...
	public static void FifthPassShift (File file) throws FileNotFoundException
	{
		...
		Scanner input = new Scanner(file);
		LightHandedList adjShift	= new LightHandedList();
		double beforeTime	= System.currentTimeMillis();
	...
	public static void SeventhPassHash(File file) throws FileNotFoundException
	{
		Scanner input = new Scanner(file);		// creates a new Scanner
		HashList HashList	= new HashList();	// Makes an instance of the HashList class
		double beforeTime	= System.currentTimeMillis();	// gets the time before its starting storing values in the table
		
		while (input.hasNext())									
			HashList.insert(cutPunctuation(input.next().toLowerCase()));
		
		double afterTime	= System.currentTimeMillis();	// time after everything is completed 
		
		System.out.print("\"Hasing List Creation\"\n");
		printStats(HashList, beforeTime, afterTime);	    // once we have processed out list we can send it to the printStats method
		
		input.close();	//closes the scanner
	} // end SeventhPassHash
	
	public static void printStats(DefaultList list, double before, double after)
	{
	...
	public static void printRemoveStats(DefaultList list , double before, double after)
	{
